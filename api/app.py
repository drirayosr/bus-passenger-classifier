...
def prepare_input_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "user_id" not in df.columns:
        ...
        df["user_id"] = (
            pd.to_numeric(df["user_id"], errors="coerce").fillna(0).astype(int)
        )
...
async def startup_event():
    ...
    try:
        ...
        if use_local_model and Path(local_model_path).exists():
            ...
            if model is None:
                print(
                    "[WARNING] No production model found! API will run in limited mode."
                )
                model_info = {
                    "model_name": "bus-passenger-classifier",
                    "version": "N/A",
                ...
                # Get model info
                model_version = registry.get_model_info(
                    "bus-passenger-classifier", stage="Production"
                )
...
@app.post(
    "/predict/single", response_model=SinglePredictionResponse, tags=["Predictions"]
)
async def predict_single(request: SinglePredictionRequest):
    ...
    try:
        ...
        if "pca_dbscan_cluster" not in result.columns:
            metrics["errors_total"] += 1
            raise HTTPException(
                status_code=500, detail="Model output missing expected columns"
            )
    ...
    try:
        ...
        for idx, row in result.iterrows():
            pred = PredictionResult(
                ...
                confidence=(
                    float(np.linalg.norm(row[pca_cols].values)) if pca_cols else None
                ),
            )
        ...
        return BatchPredictionResponse(
            predictions=predictions, summary=summary, model_info=model_info
        )
        ...
        return JSONResponse(
            content={
                ...
                "summary": {
                    ...
                    "class_distribution": output["predicted_label"]
                    .value_counts()
                    .to_dict(),
                },
                "model_info": model_info,
