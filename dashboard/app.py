...
with st.sidebar:
    ...
    page = st.radio(
        "Select Page",
        ["🏠 Home", "📊 Data Explorer", "🗺️ Map View", "📈 Model Performance", "🔮 Prediction Tool", "📡 API Monitor"],
    ...
    if api_healthy:
        st.markdown('<div class="success-box">✅ API Online</div>', unsafe_allow_html=True)
    ...
    st.caption("Real-time bus passenger classification using GPS data and machine learning.")
    st.caption("Built with MLflow, DVC, FastAPI & Streamlit")

# Main content
st.markdown('<h1 class="main-header">🚌 Bus Passenger Classifier Dashboard</h1>', unsafe_allow_html=True)
...
if page == "🏠 Home":
    ...
    with col2:
        ...
        if metrics:
            perf_data = {
                "Metric": ["Accuracy", "F1 Score", "Precision (OUT)", "Recall (OUT)", "Precision (IN)", "Recall (IN)"],
                "Value": [
...
# ========================
    ...
    if passengers_df is None:
        st.error("Unable to load passenger data. Please check if passengers.csv exists.")
    else:
        # Tabs for different views
        tab1, tab2, tab3, tab4 = st.tabs(["📈 Distributions", "🕐 Time Analysis", "👥 User Analysis", "📋 Raw Data"])
        ...
        with tab3:
            ...
            with col2:
                ...
                st.metric("Avg Records/User", f"{len(passengers_df) / passengers_df['id'].nunique():.1f}")
                st.metric("Max Records (Single User)", passengers_df['id'].value_counts().max())
...
# ========================
    ...
    if passengers_df is None:
        ...
        sample_size = st.slider("Sample Size", 100, 10000, 1000, help="Number of points to display")
...
# ========================
    ...
    if metrics is None:
        st.warning("Model metrics not found. Train a model first using `python src/train_mlflow.py`")
    else:
        col1, col2, col3, col4 = st.columns(4)
...
# ========================
    ...
    if not api_healthy:
        ...
        with col1:
            user_id = st.text_input("User ID", value="dashboard_test", help="Unique identifier for the user")
            lat = st.number_input(
                "Latitude",
                min_value=-90.0,
                max_value=90.0,
        ...
        if st.button("🔮 Make Prediction", type="primary", width='stretch'):
            ...
            if success:
                ...
                if predicted_label == 0:
                    st.markdown('<div class="error-box"><h2>🔴 OUT - Passenger Getting Off</h2></div>', unsafe_allow_html=True)
                elif predicted_label == 1:
                    st.markdown('<div class="success-box"><h2>🟢 IN - Passenger Getting On</h2></div>', unsafe_allow_html=True)
                else:
...
# ========================
    ...
    if not api_healthy:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if metric_values:
                        ...
                        with col1:
                            st.metric("Total Predictions", int(metric_values.get('predictions_total', 0)))
                        with col2:
                            st.metric("Total API Calls", int(metric_values.get('api_calls_total', 0)))
                        with col3:
                            st.metric("Total Errors", int(metric_values.get('errors_total', 0)))
                else:
        ...
        endpoints = [
            ...
            {"Method": "GET", "Endpoint": "/model/info", "Description": "Model information"},
            {"Method": "POST", "Endpoint": "/predict/single", "Description": "Single prediction"},
            {"Method": "POST", "Endpoint": "/predict/batch", "Description": "Batch predictions"},
            {"Method": "POST", "Endpoint": "/predict/csv", "Description": "CSV upload"},
            {"Method": "GET", "Endpoint": "/metrics", "Description": "Prometheus metrics"},
