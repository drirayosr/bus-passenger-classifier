...
def load_dataset(path: str, role_hint: str) -> pd.DataFrame:
    ...
    if role_hint == "phone":
        uc = detect_column(df, ["user_id", "userid", "uid", "device_id", "phone_id", "user", "id"])
...
def detect_bus_stops(
    ...
    door_open_bus = bus_df[bus_df['door_state'].isin(door_open_states)][['lon', 'lat']].copy()
    ...
    median_coords = door_open_bus[door_open_bus['cluster'].isin(top_clusters)].groupby('cluster')[['lon', 'lat']].median()
...
def load_and_preprocess_data(
    ...
    if aoi_bounds:
        ...
        print(f"  Passengers in AOI: {phones_in_aoi:,} / {len(phones):,} ({phones_in_aoi/len(phones)*100:.1f}%)")
        print(f"  Bus in AOI: {bus_in_aoi:,} / {len(bus):,} ({bus_in_aoi/len(bus)*100:.1f}%)")
