        ...
        try:
            ...
            for metric in ['f1_score', 'accuracy', 'precision_out', 'recall_out']:
                if metric in run1.data.metrics and metric in run2.data.metrics:
                    ...
                    print(f"  {metric:20s}: {v1_val:.4f} vs {v2_val:.4f} ({symbol} {abs(diff):.4f})")
...
def register_latest_model():
    ...
    if model_version:
        ...
        print(f"     python registry_manager.py --promote {model_version.version} --stage Staging")
        print("  3. After validation, promote to Production:")
        print(f"     python registry_manager.py --promote {model_version.version} --stage Production")
