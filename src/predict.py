    ...
    try:
        ...
        if 'pca_dbscan_cluster' in result.columns:
            ...
            if 'pca_component_1' in result.columns:
                ...
                predictions['confidence'] = np.linalg.norm(result[pca_cols].values, axis=1)
...
def main():
    parser = argparse.ArgumentParser(description="Make predictions with registered model")
    
    parser.add_argument('--input', type=str, help='Input CSV file path')
    parser.add_argument('--output', type=str, help='Output CSV file path')
    parser.add_argument('--model-name', type=str, default='bus-passenger-classifier',
