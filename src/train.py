...
def train_and_evaluate():
    ...
    print(
        f"  Class 0 (OUT): {(phones['labelEnc2'] == 0).sum():,} ({(phones['labelEnc2'] == 0).sum() / len(phones) * 100:.1f}%)"
    )
    print(
        f"  Class 1 (IN): {(phones['labelEnc2'] == 1).sum():,} ({(phones['labelEnc2'] == 1).sum() / len(phones) * 100:.1f}%)"
    )
    ...
    try:
        ...
        if len(X_transformed) != len(phones):
            print(
                f"[WARNING] Pipeline dropped {len(phones) - len(X_transformed)} rows ({(len(phones) - len(X_transformed)) / len(phones) * 100:.1f}%)"
            )
        ...
        results = {
            ...
            "n_features": (
                X_transformed.shape[1] if hasattr(X_transformed, "shape") else None
            ),
            "config": config,
        }
