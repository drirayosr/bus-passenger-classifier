...
def train_and_evaluate_with_mlflow(
    experiment_name="bus_passenger_classification", run_name=None
):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.log_params(
            {
                ...
                "hdbscan_min_cluster_size": config["feature_engineering"][
                    "hdbscan_min_cluster_size"
                ],
                "hdbscan_min_samples": str(
                    config["feature_engineering"]["hdbscan_min_samples"]
                ),
                "hdbscan_cluster_selection_epsilon": config["feature_engineering"][
                    "hdbscan_cluster_selection_epsilon"
                ],
                "hdbscan_cluster_selection_method": config["feature_engineering"][
        ...
        print(
            f"  Class 0 (OUT): {n_out_true:,} ({n_out_true / len(phones) * 100:.1f}%)"
        )
        ...
        try:
            ...
            if len(X_transformed) != len(phones):
                ...
                print(
                    f"[WARNING] Pipeline dropped {dropped} rows ({dropped / len(phones) * 100:.1f}%)"
                )
            ...
            print(
                f"    Class 0 (OUT): {(y_true == 0).sum():,} ({(y_true == 0).sum() / len(y_true) * 100:.1f}%)"
            )
            print(
                f"    Class 1 (IN): {(y_true == 1).sum():,} ({(y_true == 1).sum() / len(y_true) * 100:.1f}%)"
            )
            ...
            print(
                classification_report(
                    y_true, y_pred, target_names=["OUT (0)", "IN (1)"], digits=4
                )
            )
            ...
            f1_out = (
                2 * (precision_out * recall_out) / (precision_out + recall_out)
                if (precision_out + recall_out) > 0
                else 0
            )
            ...
            f1_in = (
                2 * (precision_in * recall_in) / (precision_in + recall_in)
                if (precision_in + recall_in) > 0
                else 0
            )

            print(f"\nPer-Class Metrics:")
            print(
                f"  OUT - Precision: {precision_out:.4f}, Recall: {recall_out:.4f}, F1: {f1_out:.4f}"
            )
            print(
                f"  IN  - Precision: {precision_in:.4f}, Recall: {recall_in:.4f}, F1: {f1_in:.4f}"
            )
