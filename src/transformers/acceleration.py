...
class AccelerationTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not required_cols.issubset(out.columns):
            print(
                f"Warning: Required columns {required_cols} not present in DataFrame."
            )
            return out
        ...
        def compute_acceleration_for_group(group):
            ...
            group = (
                group.sort_values(by=["timestamp_utc"], ascending=True)
        ...
        processed_groups = [
            compute_acceleration_for_group(group) for _, group in out.groupby("user_id")
        ]
    ...
    sample_data = pd.DataFrame(
        {
            ...
            "timestamp_utc": pd.to_datetime(
                ["2020-01-22 10:00:00", "2020-01-22 10:00:10", "2020-01-22 10:00:20"],
                utc=True,
            ),
