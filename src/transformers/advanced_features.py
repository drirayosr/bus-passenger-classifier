...
class AdvancedFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if "bearing" in X.columns and "user_id" in X.columns:
            ...
            X["bearing_stability"] = (
                X.groupby("user_id")["bearing_change_abs"]
                .transform(
                    lambda x: x.rolling(window=self.window_size, min_periods=1).mean()
                )
                .fillna(0)
        ...
        if self.bus_df is not None and "latitude" in X.columns:
            ...
            if sample_rows < len(X):
                print(
                    f"    ⚠️  Sampling {sample_rows} rows for multi-bus proximity (expensive)"
                )

            sample_indices = np.random.choice(X.index, sample_rows, replace=False)
            X_sample = X.loc[sample_indices]

            multi_distances = X_sample.apply(
                get_nearest_buses, axis=1, result_type="expand"
            )
            multi_distances.columns = ["dist_bus_1", "dist_bus_2", "dist_bus_3"]

            X = X.join(multi_distances)
            X[["dist_bus_1", "dist_bus_2", "dist_bus_3"]] = X[
                ["dist_bus_1", "dist_bus_2", "dist_bus_3"]
            ].fillna(1000)

            # Aggregate features
            X["min_bus_distance"] = X[["dist_bus_1", "dist_bus_2", "dist_bus_3"]].min(
                axis=1
            )
            X["avg_bus_distance"] = X[["dist_bus_1", "dist_bus_2", "dist_bus_3"]].mean(
                axis=1
            )
        ...
        if all(col in X.columns for col in ["accx", "accy", "accz"]):
            ...
            if "user_id" in X.columns:
                X["acc_rolling_std"] = (
                    X.groupby("user_id")["acc_magnitude"]
                    .transform(
                        lambda x: x.rolling(
        ...
        print(
            f"  Total new features added: {len([col for col in X.columns if col not in ['timestamp_dt', 'time_of_day']])}"
        )
...
def add_advanced_features_to_pipeline():
    ...
    print(
        "   ('advanced_features', AdvancedFeaturesTransformer(bus_df=bus_df, window_size=5)),"
    )
