...
class DistanceToBusesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for bus_id in self.bus_ids:
            ...
            bus_filtered = bus_filtered.sort_values(by="timestamp_utc").reset_index(
                drop=True
            )
    ...
    phone_data = pd.DataFrame(
        {
            ...
            "lon": [12.5230, 12.5232],
            "timestamp_utc": pd.to_datetime(
                ["2020-01-22 10:00:02", "2020-01-22 10:00:12"], utc=True
            ),
        }
    )

    transformer = DistanceToBusesTransformer(
        bus_df=bus_data, bus_ids=["bus1", "bus2"], tolerance="60s"
    )
