...
def test_hdbscan_config(phones, bus, bus_stops, config, min_cluster_size, min_samples=None, 
    ...
    try:
        ...
        config_copy['feature_engineering']['hdbscan_min_cluster_size'] = min_cluster_size
        if min_samples is not None:
            config_copy['feature_engineering']['hdbscan_min_samples'] = min_samples
        config_copy['feature_engineering']['hdbscan_cluster_selection_epsilon'] = cluster_selection_epsilon
        config_copy['feature_engineering']['hdbscan_cluster_selection_method'] = cluster_selection_method
        ...
        for cluster, count in zip(unique_clusters, counts):
            print(f"  Cluster {cluster}: {count} samples ({count/len(y_pred)*100:.1f}%)")
...
def tune_hdbscan():
    ...
    param_grid = [
        # Original settings
        {'min_cluster_size': 1000, 'min_samples': None, 'epsilon': 0.0, 'method': 'eom'},
        ...
        # Try leaf method
        {'min_cluster_size': 500, 'min_samples': None, 'epsilon': 0.0, 'method': 'leaf'},
        {'min_cluster_size': 300, 'min_samples': None, 'epsilon': 0.0, 'method': 'leaf'},
    ]
    ...
    for i, result in enumerate(results, 1):
        print(f"{i:<6}{result['f1_score']:<12.4f}{result['min_cluster_size']:<12}"
    ...
    if results:
        ...
        print(f"Predictions - Class 0: {best['pred_class_0']}, Class 1: {best['pred_class_1']}")
