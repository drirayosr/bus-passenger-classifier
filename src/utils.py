...
def compute_aoi_bounds(
    ...
    m_per_deg_lat = 111132.92 - 559.82 * np.cos(2 * np.radians(median_lat)) + \
                    1.175 * np.cos(4 * np.radians(median_lat)) - \
                    0.0023 * np.cos(6 * np.radians(median_lat))
    
    m_per_deg_lon = 111412.84 * np.cos(np.radians(median_lat)) - \
...
def time_coverage(df: pd.DataFrame, name: str) -> np.ndarray:
    ...
    print(f"{name} — sampling Δt (s) median={np.nanmedian(dt):.3f}, "
